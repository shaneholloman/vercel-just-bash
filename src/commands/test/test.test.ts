...
describe("test command", () => {
  describe("file tests", () => {
    it("-e returns 0 for existing file", async () => {
      const env = new BashEnv({ files: { "/file.txt": "content" } });
    ...
    it("-f returns 1 for directory", async () => {
      const env = new BashEnv({ files: { "/dir/file.txt": "content" } });
    ...
    it("-s returns 1 for empty file", async () => {
      const env = new BashEnv({ files: { "/empty.txt": "" } });
  ...
  describe("conditional execution", () => {
    ...
    it("[ ] works with && and ||", async () => {
      const env = new BashEnv({ files: { "/dir/file": "x" } });
